...
<meta name="description" content="Nepal's #1 Nepali AI voice generator and text to speech platform. Generate professional Nepali voiceovers for TikTok, YouTube, news and ads using 30 natural AI voices. Free preview, no signup needed. Pay with eSewa or Khalti. Try free at meroadai.com" />
...
<div style={{flex:1,height:500,overflow:'hidden',position:'relative',maskImage:'linear-gradient(to bottom, transparent 0%, black 8%, black 92%, transparent 100%)',WebkitMaskImage:'linear-gradient(to bottom, transparent 0%, black 8%, black 92%, transparent 100%)'}}>
  <div id="review-scroll-container" style={{display:'flex',flexDirection:'column',gap:16,position:'absolute',top:0,left:0,right:0}}>
    {[
      ...
      <div key={i} style={{background:'#f8f8f8',border:'1px solid #e8e8ed',borderRadius:16,padding:'16px 20px',flexShrink:0}}>
        <div style={{fontSize:13,marginBottom:8}}>⭐⭐⭐⭐⭐</div>
        <p style={{fontSize:13,color:'#1d1d1f',lineHeight:1.7,marginBottom:12,fontFamily:'Noto Sans Devanagari, Manrope, sans-serif'}}>"{r.text}"</p>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <div style={{width:28,height:28,borderRadius:'50%',background:r.color,display:'flex',alignItems:'center',justifyContent:'center',fontSize:12,fontWeight:700,color:'#fff',flexShrink:0}}>{r.name[0]}</div>
...
<div style={{display:'grid',gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)',gap:16,marginBottom:20,alignItems:'start'}}>
  ...
  <div style={{background:'linear-gradient(135deg,#7A5C08 0%,#C9940A 40%,#92700A 100%)',borderRadius:20,padding:'28px 24px',border:'2px solid rgba(255,215,0,.5)',position:'relative',boxShadow:'0 8px 40px rgba(201,148,10,.3)',transform:isMobile?'none':'scale(1.03)'}}>
    ...
    <div style={{display:'inline-flex',alignItems:'center',gap:6,background:'rgba(255,255,255,.15)',borderRadius:8,padding:'4px 10px',fontSize:12,fontWeight:700,color:'#fff',marginBottom:16}}>
      150 Videos • Lifetime Access
    </div>
    <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
      {['30,000 Swor Credits (Lifetime Pool)','5 AI Background Music Tracks (30s each)','Unlimited subtitles forever','Credits NEVER expire','Exclusive Founders Status & Badge','Commercial usage rights'].map(f=>(
    ...
    <div style={{fontSize:13,fontWeight:700,color:'rgba(255,255,255,.8)',marginBottom:16,background:'rgba(255,255,255,.1)',padding:'8px 12px',borderRadius:8}}>
      150 viral videos in your pocket. 5 original soundtracks. One payment. Forever.
    </div>
    <div style={{display:'flex',flexDirection:'column',gap:8}}>
  ...
  <div style={{background:'#fff',borderRadius:20,padding:'28px 24px',border:'1.5px solid #e8e8ed',position:'relative'}}>
    ...
    <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
      {['8,000 Swor Credits — never expire',
'~16 TikTok/Reels OR 6 YouTube Intros','Unlimited subtitles forever','All 20+ Nepali AI voices included','Commercial usage rights'].map(f=>(
        <div key={f} style={{fontSize:13,color:'#555',display:'flex',alignItems:'flex-start',gap:6}}>
          <span style={{color:'#34C759',fontWeight:700,flexShrink:0}}>✓</span> {f}
        </div>
      ))}
