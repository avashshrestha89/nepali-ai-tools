...
export default function Tool() {
  ...
  // Auth state
  const [session, setSession] = useState(null) // null = loading, false = anon, object = logged in
  ...
  // Dynamic char limit based on login state
  ...
  async function handleVoiceover() {
    ...
    try {
      ...
      if (session) {
        ...
        // Update session remaining count
  ...
  const gensLeft = session
    ? session.generationsRemaining
    : ANON_GEN_LIMIT - anonGens
      ...
      <div style={{maxWidth:680,margin:'40px auto',padding:'0 20px 80px'}}>
        ...
        <div style={{textAlign:'center',marginBottom:32}}>
          ...
          <p style={{fontSize:13,color:'#666',lineHeight:1.6}}>
            Generate Nepali subtitles and voiceovers using AI.
          </p>
        </div>
        ...
        {activeTab==='subtitle' && (
          <div style={{background:'#fff',borderRadius:16,border:'0.5px solid #E0DDD7',padding:28}}>
            <div
              ...
              style={{border:`2px dashed ${subDragOver?'#DC143C':subFile?'#1A9E6A':'#D0CCC5'}`,borderRadius:12,padding:'32px 20px',textAlign:'center',cursor:'pointer',marginBottom:20,background:subDragOver?'#FFF8F8':subFile?'#F0FAF5':'#FAFAF8'}}
            >
              <input ref={subInputRef} type="file" accept="video/*,audio/*,.mp4,.mov,.mp3,.wav,.m4a" style={{display:'none'}} onChange={e=>setSubFile(e.target.files[0])} />
              <div style={{fontSize:28,marginBottom:8}}>{subFile?'✅':'🎬'}</div>
              {subFile ? (
                <>
                  <div style={{fontSize:14,fontWeight:600,color:'#1A9E6A'}}>{subFile.name}</div>
                  <div style={{fontSize:12,color:'#888',marginTop:4}}>{(subFile.size/(1024*1024)).toFixed(1)} MB — Click to change</div>
                </>
              ) : (
                <>
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:'flex',gap:8}}>
                {[{id:'devanagari',label:'देवनागरी',sub:'Nepali script'},{id:'romanized',label:'Romanized',sub:'English letters'}].map(s=>(
                  <button key={s.id} onClick={()=>setSubStyle(s.id)} style={{flex:1,padding:'10px 14px',borderRadius:10,border:`1.5px solid ${subStyle===s.id?'#DC143C':'#E0DDD7'}`,background:subStyle===s.id?'#FFF5F5':'#FAFAF8',cursor:'pointer'}}>
            ...
            <button onClick={handleTranscribe} disabled={!subFile||subLoading}
              ...
              {subLoading?'⏳ Generating subtitles...':'✨ Generate Nepali Subtitles'}
            </button>
            ...
            <div style={{marginTop:16,fontSize:11,color:'#aaa',lineHeight:1.6,borderTop:'1px solid #F0EDE8',paddingTop:14}}>
              <strong style={{color:'#888'}}>How to use:</strong> In CapCut → Subtitles → Import → select the .SRT file.
            </div>
          </div>
        )}
        ...
        {activeTab==='voiceover' && (
          <div style={{background:'#fff',borderRadius:16,border:'0.5px solid #E0DDD7',padding:28}}>
            ...
            {/* Locked state — anonymous used up */}
            {isLocked ? (
              <div style={{textAlign:'center',padding:'40px 20px'}}>
                ...
                <p style={{fontSize:14,color:'#666',lineHeight:1.7,marginBottom:28,maxWidth:340,margin:'0 auto 28px'}}>
                  Sign up free to get <strong>10 voiceover generations</strong> with 500 characters each — during our beta period.
                </p>
              ...
              <div style={{textAlign:'center',padding:'40px 20px'}}>
                ...
                <p style={{fontSize:14,color:'#666',lineHeight:1.7,marginBottom:28,maxWidth:340,margin:'0 auto 28px'}}>
                  You've used all 10 beta generations. Subscribe to continue using Swor AI.
                </p>
